...
export default function Create({ cars, locations, main_base_id }) {
    ...
    const mainBases = useMemo(() => (locations || []).filter(l => l.type === 'main'), [locations]);
    const destinations = useMemo(() => (locations || []).filter(l => l.type === 'destination'), [locations]);

    useEffect(() => {
        if (!form.data.origin_location_id) {
            if (main_base_id) {
                form.setData('origin_location_id', String(main_base_id));
            } else if (mainBases.length === 1) {
                form.setData('origin_location_id', String(mainBases[0].id));
            }
        }
    }, [main_base_id, mainBases.length]);
        ...
        <AppLayout title="New Route">
            ...
            <div className="bg-white border rounded p-4 max-w-2xl">
                <form
                    ...
                    onSubmit={e => {
                        e.preventDefault();
                        const origin = locationsById.get(String(form.data.origin_location_id));
                        const destination = locationsById.get(String(form.data.destination_location_id));

                        if (!origin || origin.type !== 'main') {
                            alert('Origin must be Main base.');
                            return;
                        }
                        if (!destination || destination.type !== 'destination') {
                            alert('Destination must NOT be Main base.');
                            return;
                        }
                ...
                >
                    <div>
                        ...
                        {form.errors.car_id ? <div className="text-sm text-red-600 mt-1">{form.errors.car_id}</div> : null}
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label className="text-sm">Origin (Home Base)</label>
                            <select
                                className="mt-1 w-full border rounded px-3 py-2"
                                value={form.data.origin_location_id}
                                onChange={e => form.setData('origin_location_id', e.target.value)}
                                disabled={mainBases.length <= 1}
                            >
                                <option value="">Select</option>
                                {mainBases.map(l => (
                            ...
                            {form.errors.origin_location_id ? (
                                <div className="text-sm text-red-600 mt-1">{form.errors.origin_location_id}</div>
                            ) : null}
                        </div>
                        <div>
                            ...
                            <select
                                ...
                                onChange={e => {
                                    ...
                                    const next = locationsById.get(String(nextId));
                                    if (next && next.type === 'main') {
                                        alert('Destination must NOT be Main base.');
                                        form.setData('destination_location_id', '');
                                        return;
                                    }
                                    form.setData('destination_location_id', nextId);
                                }}
                            >
                                <option value="">Select</option>
                                {destinations.map(l => (
                            ...
                            {form.errors.destination_location_id ? (
                                <div className="text-sm text-red-600 mt-1">{form.errors.destination_location_id}</div>
                            ) : null}
                        </div>
                    </div>
                    ...
                    <div className="border rounded p-3">
                        <div className="flex items-start justify-between gap-3">
                            <div>
                                <div className="font-medium">Outbound Cargos</div>
                    ...
                    <div className="border rounded p-3">
                        <div className="flex items-start justify-between gap-3">
                            <div>
                                <div className="font-medium">Outbound Expenses</div>
                    ...
                    <div className="border rounded p-3">
                        <div className="font-medium">Profit Preview</div>
                        <div className="text-sm text-gray-600 mt-1">
                            Cargo {money(cargoTotal)} − Expenses {money(expenseTotal)} = <span className="font-medium">{money(profit)}</span>
                        </div>
                    </div>
                                        ...
                                        return (
                                            <div key={idx} className="p-4 flex items-start justify-between gap-3">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <div className="text-sm font-semibold text-[#0f172a] w-28 text-right">
                                                        {money(total)}
                                                    </div>
                                                    <button
                                                        type="button"
